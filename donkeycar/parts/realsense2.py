...
class RS_T265_StereoRectified(object):
    ...
    def poll(self):
		...
		# crop ROI to look at the road
                ...
                crop_height = crop_height - 20 # 20 px as padding above horizon
                grey_a = img_l[crop_height:height,0:width]
                grey_b = img_r[crop_height:height,0:width]
                grey_c = disp_vis[crop_height:height,0:width]
                
                stereo_image = np.zeros([height - crop_height, width, 3], dtype=np.dtype('B'))
                stereo_image[...,0] = np.reshape(grey_a, (height - crop_height, width))
                stereo_image[...,1] = np.reshape(grey_b, (height - crop_height, width))
                stereo_image[...,2] = np.reshape(grey_c, (height - crop_height, width))
